...
Another piece of the puzzle is population change. My understanding is that the current allocation have just been scaled down since the amounts were decided around 10 years ago, so some diverging population growth would help explain why funding has drifted away from other metrics.
...
zip_pop <- get_census(population_05_5yr, "zip", var_name = "population")
...
zip_pop %<>%
  ...
  mutate(
    year = year + 2,
    population = population * pct_pop_in_county / 100)

zip_pop_change <- zip_pop %>%
  group_by(zip) %>%
...
cm_pop <- zip_pop %>%
  select(zip, year, population) %>%
  left_join(zip_ministry_crosswalk, by = "zip") %>%
  mutate(population = population * zip_pct_in_ministry) %>%
  group_by(ministry, year) %>%
  reframe(population = sum(population, na.rm=TRUE))

cm_pop_change <- cm_pop %>%
  group_by(year) %>%
  mutate(total_pop = sum(population, na.rm = TRUE)) %>%
  ungroup() %>%
  group_by(ministry)  %>%
...
ministries %<>%
  left_join(cm_pop_change, by = "ministry")


pal <- 
  leaflet::colorNumeric(
    palette = RColorBrewer::brewer.pal(11, "RdYlGn"),
    domain = range(ministries$population_change))

leaflet(ministries) %>%
  addPolygons(
    color = "#444444", weight = 1, smoothFactor = 0.5, opacity = 1.0, fillOpacity = 0.5,
    label = paste0("Population Change 2011 to 2022: ", label_percent(accuracy = 0.1)(ministries$population_change / 100)),
    fillColor = ~pal(population_change)) %>%
    addLegend(
      pal = pal, 
      values = range(ministries$population_change), 
      opacity = 0.7, 
